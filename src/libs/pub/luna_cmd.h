#ifndef __LUNA_CMD_H__
#define __LUNA_CMD_H__

#define ACTION_STR      "action of rule: push or drop or log or tagdump\n"
#define REMOVE_STR      "remove entry\n"
#define STAT_STR        "statistics Operation\n"
#define LOAD_STR        "load configuration form file\n"
#define FILE_STR        "path of file\n"
#define DEFAULT_STR     "rule default action\n"
#define WRITE_STR       "write rule info to a file\n"


#endif /* !__LUNA_CMD_H__ */

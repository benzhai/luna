...
extern int custom_cmd_config_write(struct vty *vty);
       ...
       "Displays zebra version\n")
{
  ...
  vty_out (vty, "Compile time:%s    %s%s", VTY_NEWLINE,
           QUAGGA_COMPILE_TIME, VTY_NEWLINE);
  vty_out (vty, "%s:: Current platform version:%s    %s%s", VTY_NEWLINE,
           QUAGGA_PROGNAME, QUAGGA_VERSION, VTY_NEWLINE);
  return CMD_SUCCESS;
}
